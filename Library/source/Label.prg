...
CLASS Label INHERIT Control
   ...
   METHOD OnTimer()
   METHOD SetForeColor()
   METHOD Redraw( aRect )      INLINE ::RedrawWindow( aRect, , hb_bitor(RDW_FRAME, RDW_INVALIDATE, RDW_UPDATENOW, RDW_INTERNALPAINT) ),::UpdateWindow()
   ...
   IF ! ::DesignMode
      IF ::BlinkColor != NIL
       ...
       ELSE
         ...
         ::RedrawWindow( , , hb_bitor(RDW_INTERNALPAINT, RDW_UPDATENOW, RDW_INVALIDATE) )
      ENDIF
   ENDIF
RETURN Self
...
METHOD Label:OnPaint()
   ...
   nFlags := ::Alignment
   IF ::WrapText
      nFlags := hb_bitor(::Alignment, DT_WORDBREAK)
   ENDIF
   IF ::NoPrefix
      nFlags := hb_bitor(nFlags, DT_NOPREFIX)
   ENDIF
   ...
   IF ::VertCenter
      ...
      rc:right  := aRect[3]
      rc:bottom := aRect[4]
      DrawText( hMemDC, cText, @rc, hb_bitor(DT_CALCRECT, DT_CENTER, IIF(::WrapText, DT_WORDBREAK, 0)) )
