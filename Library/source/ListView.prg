...
METHOD SetCurSel( n ) CLASS ListView
   ...
   lvi:iItem     := n-1
   lvi:state     := hb_bitor(LVIS_SELECTED, LVIS_FOCUSED)
...
METHOD SetDataSource( oSource ) CLASS ListView
   ...
   IF VALTYPE( oSource )=="O" .AND. oSource:IsOpen
      ...
      ListViewSetItemCount( ::hWnd, oSource:RecCount(), hb_bitor(LVSICF_NOINVALIDATEALL, LVSICF_NOSCROLL))
   ENDIF
RETURN Self
...
METHOD __SetViewStyle( n ) CLASS ListView
   ...
   IF IsWindow( ::hWnd  )
      ...
      lvti:dwMask  := hb_bitor(LVTVIM_TILESIZE, LVTVIM_COLUMNS)
...
METHOD SetLVExStyle( nStyle, lAdd ) CLASS ListView
   ...
   ENDIF
   IF lAdd
      ::LvExStyle := hb_bitor(::LvExStyle, nStyle)
    ELSE
...
METHOD InsertItem( cText, nImage, nRow, nGroup, lParam ) CLASS ListView
   ...
   DEFAULT nImage TO 0
   DEFAULT nRow   TO ::SendMessage( LVM_GETITEMCOUNT, 0, 0 ) + 1
   lvi:mask       := hb_bitor(LVIF_TEXT, LVIF_IMAGE)
   IF nGroup != NIL
      lvi:mask := hb_bitor(lvi:mask, LVIF_GROUPID)
      lvi:iGroupId   := nGroup
   ENDIF
   IF lParam != NIL
      lvi:mask := hb_bitor(lvi:mask, LVIF_PARAM)
      lvi:lParam := lParam
   ENDIF
   lvi:iItem      := nRow
   lvi:iSubItem   := 0
...
METHOD Create() CLASS ListViewGroup
   ...
   plvg:mask      := hb_bitor(LVGF_HEADER, LVGF_GROUPID, LVGF_ALIGN)
