...
METHOD Create( lAdd ) CLASS MenuItem
   ...
   ::__mii:cbSize  := ::__mii:SizeOf()
   ::__mii:wID     := ::Id
   ::__mii:fMask   := hb_bitor(MIIM_DATA, MIIM_ID, MIIM_STATE, MIIM_TYPE)
   ::__mii:fType   := MFT_STRING
   ::__mii:fState  := hb_bitor(IIF( ::xEnabled, MFS_ENABLED, MFS_DISABLED ), IIF( ::xChecked, MFS_CHECKED, MFS_UNCHECKED ))

   IF ::RadioCheck
      ::__mii:fType := hb_bitor(::__mii:fType, MFT_RADIOCHECK)
   ENDIF
   IF ::RightJustified
      ::__mii:fType := hb_bitor(::__mii:fType, MFT_RIGHTJUSTIFY)
   ENDIF
   IF ::Separator .OR. ::Text == "-"
      ::__mii:fMask   := hb_bitor(MIIM_ID, MIIM_STATE, MIIM_TYPE)
   ...
   IF ::Alignment <> 0
      ::__mii:fType := hb_bitor(::__mii:fType, ::Alignment)
   ENDIF
...
METHOD __SetChecked( lCheck ) CLASS MenuItem
   ...
   IF ::__pObjPtr != NIL .AND. lCheck != ::xChecked
      mii := (struct MENUITEMINFO)
      mii:cbSize   := mii:SizeOf()
      mii:fMask    := hb_bitor(MIIM_STATE, MIIM_BITMAP)
      mii:hbmpItem := IIF( lCheck, NIL, ::__hBitmap )
      mii:fState   := IIF( lCheck, MFS_CHECKED, MFS_UNCHECKED )
      mii:fState   := hb_bitor(IIF( ::Enabled, MFS_ENABLED, MFS_DISABLED ), IIF( lCheck, MFS_CHECKED, MFS_UNCHECKED ))
      SetMenuItemInfo( ::Parent:hMenu, ::Id, .F., mii )
   ENDIF
RETURN NIL
...
METHOD __SetEnabled( lEnabled ) CLASS MenuItem
   ...
   IF ::__pObjPtr != NIL .AND. lEnabled != ::xEnabled
      ...
      mii:fState   := hb_bitor(IIF( lEnabled, MFS_ENABLED, MFS_DISABLED ), IIF( ::xChecked, MFS_CHECKED, MFS_UNCHECKED ) )
      SetMenuItemInfo( ::Parent:hMenu, ::Id, .F., mii )
   ENDIF
RETURN NIL
