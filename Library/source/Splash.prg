...
FUNCTION Splash( hInst, cImage, cType, nTimeout, aCenter, nBitmapMaskColor, nTolerance )
   ...
   nStyle  := hb_bitor(WS_POPUP, DS_SYSMODAL, WS_VISIBLE)
   nLeft   := 0
   nTop    := 0
...
METHOD MessageWait:__SetMarquee( lSet )
   ...
   IF s_lMarquee != lSet
      ...
      IF lSet
         nStyle := hb_bitor(nStyle, PBS_MARQUEE)
       ELSE
         nStyle := (nStyle & NOT( PBS_MARQUEE ))
      ENDIF
      SetWindowLong( s_hProgress, GWL_STYLE, nStyle )
      SetWindowPos( s_hProgress, ,0,0,0,0,hb_bitor(SWP_FRAMECHANGED, SWP_NOMOVE, SWP_NOSIZE, SWP_NOZORDER))
      RedrawWindow( s_hProgress, , , hb_bitor(RDW_FRAME, RDW_INVALIDATE, RDW_UPDATENOW))
...
FUNCTION __MsgWait( cText, cTitle, lProgress, cCancel, lMarquee, hParent, nMaxRange )
   ...
   nStyle := hb_bitor(WS_POPUP, WS_DLGFRAME, WS_CLIPCHILDREN)

   IF cTitle != NIL
      nStyle := hb_bitor(nStyle, WS_CAPTION, WS_THICKFRAME)
   ENDIF
   ...
   dt:style           := nStyle
   dt:dwExtendedStyle := hb_bitor(WS_EX_TOOLWINDOW, WS_EX_NOACTIVATE)
   dt:x               := 0
   dt:y               := 0
   ...
   SWITCH nMsg
      CASE WM_INITDIALOG
           ...
           IF s_cCancel != NIL
              ...
              ENDIF
              hBtn  := CreateWindowEx( 0, "Button", s_cCancel, hb_bitor(WS_CHILD, WS_VISIBLE, WS_TABSTOP, WS_CLIPCHILDREN, WS_CLIPSIBLINGS),;
                                       nLeft, aRect[4]-26, aSize[1], 22, hWnd, 4000, GetModuleHandle(), NIL )
              SendMessage( hBtn, WM_SETFONT, s_hFont )
           ENDIF
           IF s_lProgress
              aRect := _GetClientRect( hWnd )
              nStyle := hb_bitor(WS_CHILD, WS_VISIBLE, WS_TABSTOP, WS_CLIPCHILDREN, WS_CLIPSIBLINGS)
              IF s_lMarquee
                 nStyle := hb_bitor(nStyle, PBS_MARQUEE)
              ENDIF
      ...
      CASE WM_PAINT
           ...
           _DrawText( hDC, s_cText, s_aRect, hb_bitor(DT_CENTER, DT_VCENTER, DT_NOPREFIX) )
